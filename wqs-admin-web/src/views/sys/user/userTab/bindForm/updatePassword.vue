	...
	<div>
		<a-modal title="修改密码" :width="400" :open="visible" :destroy-on-close="true" @cancel="onClose">
			...
			<a-form v-else ref="formRef" :model="formState" :rules="formRules" layout="vertical">
				<div v-if="updatePasswordConfig.WQS_SYS_DEFAULT_PASSWORD_UPDATE_VALID_TYPE_FOR_B === 'OLD'">
				...
				<a-form-item
					name="phone"
					has-feedback
					v-if="updatePasswordConfig.WQS_SYS_DEFAULT_PASSWORD_UPDATE_VALID_TYPE_FOR_B === 'PHONE'"
				>
				...
				<a-form-item
					name="email"
					has-feedback
					v-if="updatePasswordConfig.WQS_SYS_DEFAULT_PASSWORD_UPDATE_VALID_TYPE_FOR_B === 'EMAIL'"
				>
				...
				<a-form-item
					name="validCode"
					v-if="
						updatePasswordConfig.WQS_SYS_DEFAULT_PASSWORD_UPDATE_VALID_TYPE_FOR_B === 'PHONE' ||
						updatePasswordConfig.WQS_SYS_DEFAULT_PASSWORD_UPDATE_VALID_TYPE_FOR_B === 'EMAIL'
					"
				>
					<a-row :gutter="8">
						<a-col :span="17">
...
<script setup name="updatePassword">
		...
		newPassword: [
			required('请输入新密码'),
			{
				validator: (rule, value) => {
					...
					// 检查密码长度
					const minLength = updatePasswordConfig.value.WQS_SYS_DEFAULT_PASSWORD_MIN_LENGTH_FOR_B
					const maxLength = updatePasswordConfig.value.WQS_SYS_DEFAULT_PASSWORD_MAX_LENGTH_FOR_B
					...
					// 检查连续相同字符
					const maxSameChar =
						updatePasswordConfig.value.WQS_SYS_DEFAULT_PASSWORD_NOT_ALLOW_CONTINUOUS_SAME_CHARACTER_LENGTH_FOR_B
					let maxCount = 1
					...
					// 检查密码复杂度
					const complexity = updatePasswordConfig.value.WQS_SYS_DEFAULT_PASSWORD_COMPLEXITY_FOR_B
	...
	const getValidCode = () => {
		formRef.value
			.validateFields([
				updatePasswordConfig.value.WQS_SYS_DEFAULT_PASSWORD_UPDATE_VALID_TYPE_FOR_B === 'PHONE' ? 'phone' : 'email'
			])
			.then(() => {
	...
	const onSubmit = async () => {
		formRef.value
			...
			.then(() => {
				...
				if (updatePasswordConfig.value.WQS_SYS_DEFAULT_PASSWORD_UPDATE_VALID_TYPE_FOR_B === 'OLD') {
				...
				} else if (updatePasswordConfig.value.WQS_SYS_DEFAULT_PASSWORD_UPDATE_VALID_TYPE_FOR_B === 'PHONE') {
					userCenterApi
				...
				} else if (updatePasswordConfig.value.WQS_SYS_DEFAULT_PASSWORD_UPDATE_VALID_TYPE_FOR_B === 'EMAIL') {
					userCenterApi
	...
	const captchaHandleOk = () => {
		...
		updatePasswordFormModalRef.value.validate().then(() => {
			...
			if (updatePasswordConfig.value.WQS_SYS_DEFAULT_PASSWORD_UPDATE_VALID_TYPE_FOR_B === 'PHONE') {
