...
<script setup>
	...
	const captchaOpen = ref(configData.SYS_BASE_CONFIG.WQS_SYS_DEFAULT_CAPTCHA_OPEN_FLAG_FOR_C)
		...
		configApi
			...
			.then((data) => {
				...
				if (data) {
					...
					captchaOpen.value = formData.value.WQS_SYS_DEFAULT_CAPTCHA_OPEN_FLAG_FOR_C
					registerOpen.value = formData.value.WQS_SYS_DEFAULT_ALLOW_REGISTER_FLAG_FOR_C
					phoneLogin.value = formData.value.WQS_SYS_DEFAULT_ALLOW_PHONE_LOGIN_FLAG_FOR_C
					emailLogin.value = formData.value.WQS_SYS_DEFAULT_ALLOW_EMAIL_LOGIN_FLAG_FOR_C
	...
	const tenCodeChange = (code) => {
		// 设置新的
		tool.data.set('WQS_TEN_CODE', code)
