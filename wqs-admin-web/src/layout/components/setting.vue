...
<script setup>
	...
	const changeTopHeaderThemeColorOpen = () => {
		...
		if (!topHeaderThemeColorOpen.value) {
			...
			tool.data.set('WQS_TOP_HEADER_THEME_COLOR_SPREAD', false)
		}
	}
	...
	const toggleState = (stateName) => {
		...
		tool.data.set(`WQS_${toolDataName}`, store[stateName])
	}
	// 设置整体风格主题
	const setSideStyle = (value) => {
		store.setTheme(value)
		tool.data.set('WQS_THEME', value)
		layoutChange(layout.value)
	}
	// 设置整体界面布局
	const layoutStyle = (value) => {
		store.setLayout(value)
		tool.data.set('WQS_LAYOUT', value)
		layoutChange(value)
	}
	// 切换颜色
	const tagColor = (value) => {
		tool.data.set('WQS_THEME_COLOR', value)
		store.setThemeColor(value)
	}
	// 切换表单风格
	const formStyleChange = (value) => {
		tool.data.set('WQS_FORM_STYLE', value)
	...
	onMounted(() => {
		const layout = tool.data.get('WQS_LAYOUT')
