...
<script setup name="baiduMap">
	...
	const emits = defineEmits(['complete', 'markerClick', 'mapClick'])
	...
	window.initMap = () => {
		...
		// 新增：地图点击事件监听
		baiduMap.value.addEventListener('click', (e) => {
			const lng = e.latlng.lng
			const lat = e.latlng.lat
			// 移除旧的点选Marker
			if (selectedMarker.value) {
				baiduMap.value.removeOverlay(selectedMarker.value)
				selectedMarker.value = null
			}
			// 创建新的Marker
			const marker = new BMapGL.Marker(new BMapGL.Point(lng, lat), { title: '点选位置' })
			baiduMap.value.addOverlay(marker)
			selectedMarker.value = marker
			// 通过事件返回坐标
			emits('mapClick', [lng, lat])
		})
	}
	...
	// 程序化添加点选标记（用于初始位置显示）
	const addSelectedMarker = (longitude, latitude, title = '选择位置') => {
		// 先清除旧的标记
		clearSelectedMarker()

		// 创建新的标记
		const marker = new BMapGL.Marker(new BMapGL.Point(longitude, latitude), { title })
		baiduMap.value.addOverlay(marker)
		selectedMarker.value = marker

		console.log('程序化添加标记成功：', longitude, latitude)
	}
