...
tool.data = {
	set(table, settings) {
		...
		const SNOWYSTRING = table.slice(0, 6) === 'WQS_' && table !== 'WQS_SYS_BASE_CONFIG'
		if (SNOWYSTRING) {
			let localSetting = JSON.parse(localStorage.getItem('WQS_SETTING')) || {}
			let newSetting = {}
			newSetting[table] = _set
			return localStorage.setItem('WQS_SETTING', JSON.stringify(Object.assign(localSetting, newSetting)))
		} else return localStorage.setItem(table, _set)
	},
	get(table) {
		const SNOWYSTRING = table.slice(0, 6) === 'WQS_' && table !== 'WQS_SYS_BASE_CONFIG'
		const WQS_SETTING = JSON.parse(localStorage.getItem('WQS_SETTING')) || {}
		let data = SNOWYSTRING ? WQS_SETTING[table] : localStorage.getItem(table)
		try {
