 ...
 */
package vip.wqs.auth.modular.login.service.impl;
...
import vip.wqs.auth.api.SaBaseLoginUserApi;
import vip.wqs.auth.core.enums.SaClientTypeEnum;
import vip.wqs.auth.core.pojo.SaBaseClientLoginUser;
import vip.wqs.auth.core.pojo.SaBaseLoginUser;
import vip.wqs.auth.core.util.AuthEmailFormatUtil;
import vip.wqs.auth.core.util.StpClientLoginUserUtil;
import vip.wqs.auth.core.util.StpClientUtil;
import vip.wqs.auth.core.util.StpLoginUserUtil;
import vip.wqs.auth.modular.login.enums.AuthDeviceTypeEnum;
import vip.wqs.auth.modular.login.enums.AuthExceptionEnum;
import vip.wqs.auth.modular.login.enums.AuthPhoneOrEmailTypeEnum;
import vip.wqs.auth.modular.login.enums.AuthStrategyWhenNoUserWithPhoneOrEmailEnum;
import vip.wqs.auth.modular.login.param.*;
import vip.wqs.auth.modular.login.result.AuthPicValidCodeResult;
import vip.wqs.auth.modular.login.service.AuthService;
import vip.wqs.common.cache.CommonCacheOperator;
import vip.wqs.common.consts.CacheConstant;
import vip.wqs.common.exception.CommonException;
import vip.wqs.common.util.CommonCryptogramUtil;
import vip.wqs.common.util.CommonEmailUtil;
import vip.wqs.common.util.CommonTimeFormatUtil;
...
public class AuthServiceImpl implements AuthService {
    ...
    /** B端验证码是否开启（适用图片验证码） */
    private static final String WQS_SYS_DEFAULT_CAPTCHA_OPEN_FLAG_FOR_B_KEY = "WQS_SYS_DEFAULT_CAPTCHA_OPEN_FLAG_FOR_B";

    /** C端验证码是否开启（适用图片验证码） */
    private static final String WQS_SYS_DEFAULT_CAPTCHA_OPEN_FLAG_FOR_C_KEY = "WQS_SYS_DEFAULT_CAPTCHA_OPEN_FLAG_FOR_C";

    /** B端验证码失效时间（适用图片验证码和短信验证码，单位：分钟，默认5分钟有效） */
    private static final String WQS_SYS_DEFAULT_CAPTCHA_EXPIRED_DURATION_FOR_B_KEY = "WQS_SYS_DEFAULT_CAPTCHA_EXPIRED_DURATION_FOR_B";
    ...
    /** B端登录验证码短信消息模板编码 */
    private static final String WQS_SMS_TEMPLATE_VALID_CODE_LOGIN_FOR_B_KEY = "WQS_SMS_TEMPLATE_VALID_CODE_LOGIN_FOR_B";

    /** C端登录验证码短信消息模板编码 */
    private static final String WQS_SMS_TEMPLATE_VALID_CODE_LOGIN_FOR_C_KEY = "WQS_SMS_TEMPLATE_VALID_CODE_LOGIN_FOR_C";

    /** B端登录验证码邮件消息模板内容 */
    private static final String WQS_EMAIL_TEMPLATE_VALID_CODE_LOGIN_FOR_B_KEY = "WQS_EMAIL_TEMPLATE_VALID_CODE_LOGIN_FOR_B";

    /** C端登录验证码邮件消息模板内容 */
    private static final String WQS_EMAIL_TEMPLATE_VALID_CODE_LOGIN_FOR_C_KEY = "WQS_EMAIL_TEMPLATE_VALID_CODE_LOGIN_FOR_C";

    /** B端连续登录失败持续时间（即N分钟内连续登录失败，单位：分钟） */
    private static final String WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_DURATION_FOR_B_KEY = "WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_DURATION_FOR_B";

    /** C端连续登录失败持续时间（即N分钟内连续登录失败，单位：分钟） */
    private static final String WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_DURATION_FOR_C_KEY = "WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_DURATION_FOR_C";

    /** B端连续登录失败次数（即指定分钟内连续登录失败N次） */
    private static final String WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_TIMES_FOR_B_KEY = "WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_TIMES_FOR_B";

    /** C端连续登录失败次数（即指定分钟内连续登录失败N次） */
    private static final String WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_TIMES_FOR_C_KEY = "WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_TIMES_FOR_C";

    /** B端连续登录失败锁定时间（即指定分钟内连续登录失败指定次数，锁定N分钟，单位：分钟） */
    private static final String WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_LOCK_DURATION_FOR_B_KEY = "WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_LOCK_DURATION_FOR_B";

    /** C端连续登录失败锁定时间（即指定分钟内连续登录失败指定次数，锁定N分钟，单位：分钟） */
    private static final String WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_LOCK_DURATION_FOR_C_KEY = "WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_LOCK_DURATION_FOR_C";

    /** B端手机号登录是否开启 */
    private static final String WQS_SYS_DEFAULT_ALLOW_PHONE_LOGIN_FLAG_FOR_B_KEY = "WQS_SYS_DEFAULT_ALLOW_PHONE_LOGIN_FLAG_FOR_B";

    /** C端手机号登录是否开启 */
    private static final String WQS_SYS_DEFAULT_ALLOW_PHONE_LOGIN_FLAG_FOR_C_KEY = "WQS_SYS_DEFAULT_ALLOW_PHONE_LOGIN_FLAG_FOR_C";

    /** B端邮箱登录是否开启 */
    private static final String WQS_SYS_DEFAULT_ALLOW_EMAIL_LOGIN_FLAG_FOR_B_KEY = "WQS_SYS_DEFAULT_ALLOW_EMAIL_LOGIN_FLAG_FOR_B";

    /** C端邮箱登录是否开启 */
    private static final String WQS_SYS_DEFAULT_ALLOW_EMAIL_LOGIN_FLAG_FOR_C_KEY = "WQS_SYS_DEFAULT_ALLOW_EMAIL_LOGIN_FLAG_FOR_C";

    /** B端手机号无对应用户时策略 */
    private static final String WQS_SYS_DEFAULT_STRATEGY_WHEN_NO_USER_WITH_PHONE_FOR_B_KEY = "WQS_SYS_DEFAULT_STRATEGY_WHEN_NO_USER_WITH_PHONE_FOR_B";

    /** C端手机号无对应用户时策略 */
    private static final String WQS_SYS_DEFAULT_STRATEGY_WHEN_NO_USER_WITH_PHONE_FOR_C_KEY = "WQS_SYS_DEFAULT_STRATEGY_WHEN_NO_USER_WITH_PHONE_FOR_C";

    /** B端邮箱无对应用户时策略 */
    private static final String WQS_SYS_DEFAULT_STRATEGY_WHEN_NO_USER_WITH_EMAIL_FOR_B_KEY = "WQS_SYS_DEFAULT_STRATEGY_WHEN_NO_USER_WITH_EMAIL_FOR_B";

    /** C端邮箱无对应用户时策略 */
    private static final String WQS_SYS_DEFAULT_STRATEGY_WHEN_NO_USER_WITH_EMAIL_FOR_C_KEY = "WQS_SYS_DEFAULT_STRATEGY_WHEN_NO_USER_WITH_EMAIL_FOR_C";

    /** B端注册是否开启 */
    private static final String WQS_SYS_DEFAULT_ALLOW_REGISTER_FLAG_FOR_B_KEY = "WQS_SYS_DEFAULT_ALLOW_REGISTER_FLAG_FOR_B";

    /** C端注册是否开启 */
    private static final String WQS_SYS_DEFAULT_ALLOW_REGISTER_FLAG_FOR_C_KEY = "WQS_SYS_DEFAULT_ALLOW_REGISTER_FLAG_FOR_C";
    ...
    public String getPhoneValidCode(AuthGetPhoneValidCodeParam authGetPhoneValidCodeParam, String type) {
        ...
        if(SaClientTypeEnum.B.getValue().equals(type)) {
            smsTemplateCode = devConfigApi.getValueByKey(WQS_SMS_TEMPLATE_VALID_CODE_LOGIN_FOR_B_KEY);
        } else {
            smsTemplateCode = devConfigApi.getValueByKey(WQS_SMS_TEMPLATE_VALID_CODE_LOGIN_FOR_C_KEY);
        }
    ...
    private void checkAllowPhoneLoginFlag(String type) {
        ...
        if(SaClientTypeEnum.B.getValue().equals(type)) {
            allowPhoneLoginFlag = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_ALLOW_PHONE_LOGIN_FLAG_FOR_B_KEY);
        } else {
            allowPhoneLoginFlag = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_ALLOW_PHONE_LOGIN_FLAG_FOR_C_KEY);
        }
    ...
    public String getEmailValidCode(AuthGetEmailValidCodeParam authGetEmailValidCodeParam, String type) {
        ...
        if(SaClientTypeEnum.B.getValue().equals(type)) {
            emailTemplateContent = devConfigApi.getValueByKey(WQS_EMAIL_TEMPLATE_VALID_CODE_LOGIN_FOR_B_KEY);
        } else {
            emailTemplateContent = devConfigApi.getValueByKey(WQS_EMAIL_TEMPLATE_VALID_CODE_LOGIN_FOR_C_KEY);
        }
    ...
    private void checkAllowEmailLoginFlag(String type) {
        ...
        if(SaClientTypeEnum.B.getValue().equals(type)) {
            allowEmailLoginFlag = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_ALLOW_EMAIL_LOGIN_FLAG_FOR_B_KEY);
        } else {
            allowEmailLoginFlag = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_ALLOW_EMAIL_LOGIN_FLAG_FOR_C_KEY);
        }
        ...
        if(SaClientTypeEnum.B.getValue().equals(type)) {
            if(phoneOrEmailType.equals(AuthPhoneOrEmailTypeEnum.PHONE.getValue())) {
                ...
                if(ObjectUtil.isEmpty(saBaseLoginUser)) {
                    // B端手机号无对应用户时策略
                    return devConfigApi.getValueByKey(WQS_SYS_DEFAULT_STRATEGY_WHEN_NO_USER_WITH_PHONE_FOR_B_KEY);
                } else {
                    // 存在则允许登录
                ...
                if(ObjectUtil.isEmpty(saBaseLoginUser)) {
                    // B端邮箱无对应用户时策略
                    return devConfigApi.getValueByKey(WQS_SYS_DEFAULT_STRATEGY_WHEN_NO_USER_WITH_EMAIL_FOR_B_KEY);
                } else {
                    // 存在则允许登录
                    return AuthStrategyWhenNoUserWithPhoneOrEmailEnum.ALLOW_LOGIN.getValue();
                }
            ...
            if(phoneOrEmailType.equals(AuthPhoneOrEmailTypeEnum.PHONE.getValue())) {
                ...
                if(ObjectUtil.isEmpty(saBaseClientLoginUser)) {
                    // C端手机号无对应用户时策略
                    return devConfigApi.getValueByKey(WQS_SYS_DEFAULT_STRATEGY_WHEN_NO_USER_WITH_PHONE_FOR_C_KEY);
                } else {
                    // 存在则允许登录
                    return AuthStrategyWhenNoUserWithPhoneOrEmailEnum.ALLOW_LOGIN.getValue();
                }
                ...
                if(ObjectUtil.isEmpty(saBaseClientLoginUser)) {
                    // C端邮箱无对应用户时策略
                    return devConfigApi.getValueByKey(WQS_SYS_DEFAULT_STRATEGY_WHEN_NO_USER_WITH_EMAIL_FOR_C_KEY);
                } else {
                    // 存在则允许登录
                    return AuthStrategyWhenNoUserWithPhoneOrEmailEnum.ALLOW_LOGIN.getValue();
                }
    ...
    public String doLogin(AuthAccountPasswordLoginParam authAccountPasswordLoginParam, String type) {
        ...
        if(SaClientTypeEnum.B.getValue().equals(type)) {
            defaultCaptchaOpen = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_CAPTCHA_OPEN_FLAG_FOR_B_KEY);
        } else {
            defaultCaptchaOpen = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_CAPTCHA_OPEN_FLAG_FOR_C_KEY);
        }
    ...
    private int saveLoginTimes(String userAccount, String type){
        ...
        if(SaClientTypeEnum.B.getValue().equals(type)) {
            configContinuousLoginFailDuration = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_DURATION_FOR_B_KEY);
            configContinuousLoginFailTimes = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_TIMES_FOR_B_KEY);
            configContinuousLoginFailLockDuration = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_LOCK_DURATION_FOR_B_KEY);
        } else {
            configContinuousLoginFailDuration = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_DURATION_FOR_C_KEY);
            configContinuousLoginFailTimes = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_TIMES_FOR_C_KEY);
            configContinuousLoginFailLockDuration = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_LOCK_DURATION_FOR_C_KEY);
        }
        // 连续登录失败持续时间默认5分钟
    ...
    @Override
    public String doLoginByMini(AuthMiniLoginParam authMiniLoginParam, String type) {
        try {
            // 获取微信小程序服务
            ...
            // 设备类型
            String device = authMiniLoginParam.getDevice();
            if (ObjectUtil.isEmpty(device)) {
            ...
            // 查找是否已有绑定的用户
            SaBaseClientLoginUser existUser = findUserByWechatOpenid(openid);
            
            if (existUser != null) {
                // 已有绑定用户，直接登录
                return execLoginC(existUser, device);
            } else {
                // 新用户，需要创建或绑定账户
                return handleNewWechatUser(authMiniLoginParam, openid, unionid, device, type);
            }
    ...
    private void checkAllowRegisterFlag(String type) {
        ...
        if(SaClientTypeEnum.B.getValue().equals(type)) {
            allowRegisterFlag = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_ALLOW_REGISTER_FLAG_FOR_B_KEY);
        } else {
            allowRegisterFlag = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_ALLOW_REGISTER_FLAG_FOR_C_KEY);
        }
    ...
    private long getValidCodeExpiredDuration(String type) {
        ...
        if(SaClientTypeEnum.B.getValue().equals(type)) {
            configCaptchaExpiredDuration = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_CAPTCHA_EXPIRED_DURATION_FOR_B_KEY);
        } else {
            configCaptchaExpiredDuration = devConfigApi.getValueByKey(WQS_SYS_DEFAULT_CAPTCHA_EXPIRED_DURATION_FOR_C_KEY);
        }
        // 判断是否为空
     ...
     * @date 2025/01/29
     */
    private SaBaseClientLoginUser findUserByWechatOpenid(String openid) {
        try {
            // 使用现有的clientLoginUserApi查找微信用户
            return clientLoginUserApi.getClientUserByWechatOpenid(openid);
        } catch (Exception e) {
            log.error("查找微信用户失败，openid: {}, error: {}", openid, e.getMessage(), e);
            return null;
        }
    }
     ...
     * @date 2025/01/29
     */
    private String handleNewWechatUser(AuthMiniLoginParam authMiniLoginParam, String openid, String unionid, String device, String type) {
        try {
            ...
            System.out.println("=== 微信登录调试信息 ===");
            System.out.println("解析到的手机号: " + phoneNumber);
            System.out.println("原始参数中的手机号: " + authMiniLoginParam.getPhone());
            System.out.println("授权码: " + authMiniLoginParam.getPhoneAuthCode());
            System.out.println("加密数据: " + authMiniLoginParam.getEncryptedData());
            System.out.println("========================");
            
            // 如果提供了手机号（直接提供或从微信授权中解析），尝试绑定已有账户
            if (StrUtil.isNotBlank(phoneNumber)) {
                System.out.println("从微信授权中解析到手机号: " + phoneNumber);
                
                // 查找手机号对应的用户
                SaBaseClientLoginUser existUser = clientLoginUserApi.getClientUserByPhone(phoneNumber);
                if (existUser != null) {
                    // 通过clientLoginUserApi绑定微信信息到已有用户
                    clientLoginUserApi.bindWechatInfo(existUser.getId(), openid, unionid, 
                        authMiniLoginParam.getNickName(), authMiniLoginParam.getAvatarUrl());
                    
                    System.out.println("微信绑定已有用户成功，userId: " + existUser.getId() + ", openid: " + openid + ", phone: " + phoneNumber);
                    return execLoginC(existUser, device);
                }
            }
            
            // 如果提供了手机号验证码方式的手机号，进行验证
            if (StrUtil.isNotBlank(authMiniLoginParam.getPhone())) {
                // 验证手机号验证码
                if (StrUtil.isBlank(authMiniLoginParam.getPhoneCode())) {
                    throw new CommonException("绑定手机号时验证码不能为空");
                }
                
                // 验证手机号验证码
                validValidCode(authMiniLoginParam.getPhone(), authMiniLoginParam.getPhoneCode(), authMiniLoginParam.getPhoneCode());
                
                // 查找手机号对应的用户
                SaBaseClientLoginUser existUser = clientLoginUserApi.getClientUserByPhone(authMiniLoginParam.getPhone());
                if (existUser != null) {
                    // 通过clientLoginUserApi绑定微信信息到已有用户
                    clientLoginUserApi.bindWechatInfo(existUser.getId(), openid, unionid, 
                        authMiniLoginParam.getNickName(), authMiniLoginParam.getAvatarUrl());
                    
                    System.out.println("微信绑定已有用户成功，userId: " + existUser.getId() + ", openid: " + openid);
                    return execLoginC(existUser, device);
                }
                
                // 使用验证码方式的手机号作为最终手机号
                phoneNumber = authMiniLoginParam.getPhone();
            }
            
            // 创建新的微信用户
            // 生成用户账号（使用openid作为账号标识）
            String account = "wx_" + openid.substring(openid.length() - 8); // 取openid后8位
            String nickname = StrUtil.isNotBlank(authMiniLoginParam.getNickName()) ? 
                authMiniLoginParam.getNickName().trim() : "微信用户";
            
            // 确保昵称不为空且长度合理
            if (StrUtil.isBlank(nickname) || nickname.length() > 50) {
                nickname = "微信用户";
            }
            
            // 通过clientLoginUserApi创建微信用户
            String userId = clientLoginUserApi.createWechatUser(account, nickname, 
                authMiniLoginParam.getAvatarUrl(), openid, unionid, phoneNumber);
            ...
            // 获取创建的用户信息
            SaBaseClientLoginUser newUser = clientLoginUserApi.getClientUserById(userId);
            if (newUser == null) {
                throw new CommonException("获取新创建的微信用户失败");
            }
            
            System.out.println("创建微信用户成功，userId: " + userId + ", openid: " + openid + ", phone: " + phoneNumber);
     ...
     * @date 2025/01/29
     */
    private String extractPhoneFromWechatAuth(AuthMiniLoginParam authMiniLoginParam, String sessionKey) {
        try {
            // 1. 如果直接提供了手机号，直接返回
            if (StrUtil.isNotBlank(authMiniLoginParam.getPhone())) {
            ...
            // 4. 尝试使用云开发ID获取手机号
            if (StrUtil.isNotBlank(authMiniLoginParam.getCloudID())) {
                log.info("尝试使用云开发ID获取手机号: {}", authMiniLoginParam.getCloudID());
                return decryptPhoneByCloudID(authMiniLoginParam.getCloudID());
            }
     ...
     * @date 2025/01/29
     */
    private String decryptPhoneByCloudID(String cloudID) {
        try {
            // 云开发环境的手机号获取逻辑待实现
            System.out.println("云开发手机号获取功能待实现，cloudID: " + cloudID);
            return null;
            
        } catch (Exception e) {
            System.out.println("通过云开发ID获取手机号失败: " + e.getMessage());
            return null;
        }
    }
}
