...
public class DevFileLocalUtil {
    ...
    private static final String WQS_FILE_LOCAL_FOLDER_FOR_WINDOWS_KEY = "WQS_FILE_LOCAL_FOLDER_FOR_WINDOWS";
    private static final String WQS_FILE_LOCAL_FOLDER_FOR_UNIX_KEY = "WQS_FILE_LOCAL_FOLDER_FOR_UNIX";
    ...
    private static void initClient() {
        ...
        if(SystemUtil.getOsInfo().isWindows()) {
            ...
            /* 本地文件上传的位置 windows系统 */
            String localFolderForWindows = devConfigApi.getValueByKey(WQS_FILE_LOCAL_FOLDER_FOR_WINDOWS_KEY);

            if(ObjectUtil.isEmpty(localFolderForWindows)) {
                throw new CommonException("本地文件操作客户端未正确配置：WQS_FILE_LOCAL_FOLDER_FOR_WINDOWS为空");
            }
            uploadFileFolder = localFolderForWindows;
        } else {

            /* 本地文件上传的位置 unix系列系统（linux、mac等） */
            String localFolderForUnix = devConfigApi.getValueByKey(WQS_FILE_LOCAL_FOLDER_FOR_UNIX_KEY);

            if(ObjectUtil.isEmpty(localFolderForUnix)) {
                throw new CommonException("本地文件操作客户端未正确配置：WQS_FILE_LOCAL_FOLDER_FOR_UNIX为空");
            }
