 ...
 */
package vip.wqs.dev.modular.email.util;
...
public class DevEmailLocalUtil {
    ...
    private static final String WQS_EMAIL_LOCAL_FROM_KEY = "WQS_EMAIL_LOCAL_FROM";

    private static final String WQS_EMAIL_LOCAL_PASSWORD_KEY = "WQS_EMAIL_LOCAL_PASSWORD";

    private static final String WQS_EMAIL_LOCAL_SMTP_HOST_KEY = "WQS_EMAIL_LOCAL_SMTP_HOST";

    private static final String WQS_EMAIL_LOCAL_SMTP_PORT_KEY = "WQS_EMAIL_LOCAL_SMTP_PORT";

    private static final String WQS_EMAIL_LOCAL_AUTH_KEY = "WQS_EMAIL_LOCAL_AUTH";

    private static final String WQS_EMAIL_LOCAL_SSL_ENABLE_KEY = "WQS_EMAIL_LOCAL_SSL_ENABLE";

    private static final String WQS_EMAIL_LOCAL_STARTTLS_ENABLE_KEY = "WQS_EMAIL_LOCAL_STARTTLS_ENABLE";
    ...
    private static void initClient() {
        ...
        /* 发件人（必须正确，否则发送失败） */
        String from = devConfigApi.getValueByKey(WQS_EMAIL_LOCAL_FROM_KEY);
        ...
        String pass = devConfigApi.getValueByKey(WQS_EMAIL_LOCAL_PASSWORD_KEY);
        ...
        /* SMTP服务器域名 */
        String host = devConfigApi.getValueByKey(WQS_EMAIL_LOCAL_SMTP_HOST_KEY);
        if (ObjectUtil.isNotEmpty(host)) {
            mailAccount.setHost(host);
        }
        /* SMTP服务端口 */
        String port = devConfigApi.getValueByKey(WQS_EMAIL_LOCAL_SMTP_PORT_KEY);
        if (ObjectUtil.isNotEmpty(port)) {
            mailAccount.setPort(Integer.parseInt(port));
        }
        /* 是否需要用户名密码验证 */
        String auth = devConfigApi.getValueByKey(WQS_EMAIL_LOCAL_AUTH_KEY);
        mailAccount.setAuth(Objects.equals(auth, "true"));

        /* 是否使用SSL安全连接 */
        String sslEnable = devConfigApi.getValueByKey(WQS_EMAIL_LOCAL_SSL_ENABLE_KEY);
        mailAccount.setSslEnable(Objects.equals(sslEnable, "true"));

        /* 是否使用STARTTLS安全连接 */
        String starttlsEnable = devConfigApi.getValueByKey(WQS_EMAIL_LOCAL_STARTTLS_ENABLE_KEY);
