 ...
 */
package vip.wqs.dev.modular.file.controller;
 ...
 **/
@Tag(name = "文件控制器")
@ApiSupport(author = "WQS_TEAM", order = 4)
@RestController
@Validated
public class DevFileController {
    ...
    public CommonResult<String> uploadDynamicReturnId(@RequestPart("file") MultipartFile file) {
        return CommonResult.data(devFileService.uploadReturnId(devConfigApi.getValueByKey(WQS_SYS_DEFAULT_FILE_ENGINE_KEY), file));
    }
    ...
    public CommonResult<String> uploadDynamicReturnUrl(@RequestPart("file") MultipartFile file) {
        return CommonResult.data(devFileService.uploadReturnUrl(devConfigApi.getValueByKey(WQS_SYS_DEFAULT_FILE_ENGINE_KEY), file));
    }
